	...
	Если ИсключатьПроверкуДомаВАдресе(АдресРФ) Тогда
		...
		Если ЗданиеНеУказано Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, 
				...
				НСтр("ru='Не указан дом или корпус.'"), "Дом");
		КонецЕсли;
...
Процедура ЗаполнитьОшибкиАдресаКЛАДР(XDTOАдресРФ, Результат)
	...
	// Проверим на соответствие частей классификатору
...
Функция XDTOКонтактнаяИнформацияЗаполнена(Знач XDTOДанные) Экспорт
	...
	Если Не ПустаяСтрока(XDTOДанные.Представление) Тогда
		Возврат Истина;
		
	ИначеЕсли Не ПустаяСтрока(XDTOДанные.Комментарий) Тогда
		Возврат Истина;
		
	Иначе
		Состав = XDTOДанные.Состав;
		Если Состав <> Неопределено Тогда
			Для Каждого СвойстваСостава Из XDTOДанные.Состав.Свойства() Цикл
				Если Состав.Установлено(СвойстваСостава) Тогда
					Возврат Истина;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
