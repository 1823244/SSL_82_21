...
Функция ПроверитьСоответствиеАдресаКЛАДРу(Знач УказанныйИндекс = "", Знач НазваниеРегиона = "", Знач НазваниеРайона = "", Знач НазваниеГорода = "", Знач НазваниеНаселенногоПункта = "", Знач НазваниеУлицы = "", Знач НомерДома = "", Знач НомерКорпуса = "") Экспорт
		...
		Если КодДома=0 Тогда
			...
			Если ДополнительнаяПроверка.ЕстьЗдания Тогда
				Если ДополнительнаяПроверка.Код<>Неопределено Тогда // Нашли
					...
					НомерИндекса = ДополнительнаяПроверка.Индекс;
	...
	// Проверка индекса 
	// Только в случае, если смогли определить по классификатору, иначе, то что введено вручную - корректно
	Если Не ПустаяСтрока(УказанныйИндекс) И Не ПустаяСтрока(НомерИндекса) И НомерИндекса <> УказанныйИндекс Тогда
...
Процедура СохранитьФайлНаСервереИРаспаковать(Знач ДвоичныеДанные, ИмяФайлаАрхива, ПутьККаталогуНаСервере) Экспорт
	...
	// При распаковке приводим имена к верхнему регистру
	Для Каждого ЭлементЗип Из ЧтениеZIP.Элементы Цикл
		ЧтениеZIP.Извлечь(ЭлементЗип, ПутьККаталогуНаСервере, РежимВосстановленияПутейФайловZIP.НеВосстанавливать);
		ПереместитьФайл(ПутьККаталогуНаСервере + ЭлементЗип.Имя, ПутьККаталогуНаСервере + ВРег(ЭлементЗип.Имя) );
	КонецЦикла;
...
Функция ВходитВИнтервал(Знач Номер, Интервал, ТочноеСоответствие)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Номер, Ложь) Тогда
		...
		Вариант = ?(ТипЗнч(Номер) = Тип("Число"), Формат(Номер, "ЧГ="), ВРег(СтрЗаменить(Номер, " ", "")));
		Позиция = Найти(Вариант, "К");
		НомерБезЛишнихСимволов = ?(Позиция > 0, Лев(Вариант, Позиция - 1), Вариант);
		
		Вариант  = ВРег(СтрЗаменить(Интервал, " ", ""));
		Позиция = Найти(Вариант, "К");
		ИнтервалБезЛишнихСимволов = ?(Позиция > 0, Лев(Вариант, Позиция - 1), Вариант);
