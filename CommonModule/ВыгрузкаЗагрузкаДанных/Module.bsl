...
Процедура ПриДобавленииСлужебныхСобытий(КлиентскиеСобытия, СерверныеСобытия) Экспорт
	...
	//
	// (То же, что ВыгрузкаЗагрузкаДанныхПереопределяемый.ТипыОбщихПредопределенныхДанных).
	//
	СерверныеСобытия.Добавить(
		"СтандартныеПодсистемы.РаботаВМоделиСервиса.ВыгрузкаЗагрузкаДанных\ПриОпределенииСтандартныхТиповОбщихПредопределенныхДанных");
...
Функция ТипыОбщихДанныхСПредопределеннымиЭлементами()
	...
	Для каждого Обработчик Из ОбработчикиСобытия Цикл
		Обработчик.Модуль.ПриОпределенииСтандартныхТиповОбщихПредопределенныхДанных(Результат);
	КонецЦикла;
	
	ВыгрузкаЗагрузкаДанныхПереопределяемый.ТипыОбщихПредопределенныхДанных(Результат);
...
Процедура ЗаписатьДанныеВXMLСУказаниемТипов(КаталогВыгрузки, ТипыДляЗамены = Неопределено, МаксимальныйРазмерЧасти = 100000, 
	...
	ТипыОбщихДанныхСПредопределеннымиЭлементами = ТипыОбщихДанныхСПредопределеннымиЭлементами();
	Для Каждого ТипОбщихДанныхСПредопределеннымиЭлементами Из ТипыОбщихДанныхСПредопределеннымиЭлементами Цикл
		
		СтрокаОбъекта = ВыгружаемыеДанные.Добавить();
		ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипОбщихДанныхСПредопределеннымиЭлементами);
		ОписаниеВыгружаемогоОбъекта = Новый Структура("ПолноеИмя,Имя,Разделитель,УсловноеРазделение",
			ОбъектМетаданных.ПолноеИмя(), ОбъектМетаданных.Имя);
		СтрокаОбъекта.ВыгружаемыйОбъект = ОписаниеВыгружаемогоОбъекта;
		
	КонецЦикла;
	НеразделенныеПредопределенные = НоваяТаблицаПредопределенных();
	НеразделенныеПредопределенные.Колонки.Добавить("ИмяТипаXML", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(256, ДопустимаяДлина.Переменная)));
	
	ИменаМетаданныхЗаменяемыхТипов = Новый Соответствие;
	Если ТипыДляЗамены <> Неопределено ИЛИ ТипыОбщихДанныхСПредопределеннымиЭлементами.Количество() > 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ТипыСАннотациейСсылок, ТипыДляЗамены);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ТипыСАннотациейСсылок, ТипыОбщихДанныхСПредопределеннымиЭлементами());
	...
	Для Каждого СтрокаОбъекта Из ВыгружаемыеДанные Цикл
		...
		ИначеЕсли РаботаВМоделиСервиса.ЭтоПолноеИмяОбъектаСсылочногоТипа(ОписаниеВыгружаемогоОбъекта.ПолноеИмя) Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ТипыОбщихДанныхСПредопределеннымиЭлементами.Найти(ТипЗнч(Выборка.Ссылка)) <> Неопределено
						И Объект.Предопределенный Тогда
					
					СтрокаПредопределенного = НеразделенныеПредопределенные.Добавить();
					СтрокаПредопределенного.Ссылка = Объект.Ссылка;
					СтрокаПредопределенного.Имя = Менеджер.ПолучитьИмяПредопределенного(СтрокаПредопределенного.Ссылка);
					СтрокаПредопределенного.ИмяТипаXML = СериализаторXDTO.XMLТип(ТипЗнч(Объект.Ссылка)).ИмяТипа;
	...
	Если ТипыОбщихДанныхСПредопределеннымиЭлементами.Количество() > 0 Тогда
		
		Запись = Новый ЗаписьXML();
		
		Запись.ОткрытьФайл(КаталогВыгрузки + "UnseparatedPredefinedDataNames.xml");
		Запись.ЗаписатьОбъявлениеXML();
		Запись.ЗаписатьНачалоЭлемента("Data");
		Запись.ЗаписатьСоответствиеПространстваИмен("xs", "http://www.w3.org/2001/XMLSchema");
		Запись.ЗаписатьСоответствиеПространстваИмен("xsi", "http://www.w3.org/2001/XMLSchema-instance");
		...
		Сериализатор.ЗаписатьXML(Запись, НеразделенныеПредопределенные);
...
Процедура ЗагрузитьДанныеСПодменойСсылок(Знач КаталогВыгрузки)
	...
	Если СтандартныеПодсистемыПовтИсп.ЭтоПлатформа83БезРежимаСовместимости() Тогда
		...
		// Дополняем словарь замен ссылками на неразделенные предопределенные
		
		ФайлСоответствия = Новый Файл(КаталогВыгрузки + "UnseparatedPredefinedDataNames.xml");
		...
		Если ФайлСоответствия.Существует() Тогда
			...
			НеразделенныеПредопределенные = СериализаторXDTO.ПрочитатьXML(Чтение);
			
			Чтение.Закрыть();
			Чтение = Неопределено;
			
			ТипыНеразделенныхПредопределенных = НеразделенныеПредопределенные.Скопировать();
			ТипыНеразделенныхПредопределенных.Свернуть("ИмяТипаXML");
			
			НеразделенныеПредопределенные.Индексы.Добавить("ИмяТипаXML");
			
			Для Каждого ТипНеразделенныхПредопределенных Из ТипыНеразделенныхПредопределенных Цикл
				
				НеразделенныеПредопределенныеТипа = НеразделенныеПредопределенные.НайтиСтроки(
					Новый Структура("ИмяТипаXML", ТипНеразделенныхПредопределенных.ИмяТипаXML));
				
				СоответствиеСсылок = Новый Соответствие();
				
				Для Каждого НеразделенныйПредопределенный Из НеразделенныеПредопределенныеТипа Цикл
					
					ТекущийПредопределенный = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент(
						НеразделенныйПредопределенный.Ссылка.Метаданные().ПолноеИмя() + "." + НеразделенныйПредопределенный.Имя);
					
					Если ТекущийПредопределенный <> Неопределено Тогда
						
						СоответствиеСсылок.Вставить(
							ТекущийПредопределенный.УникальныйИдентификатор(),
							НеразделенныйПредопределенный.Ссылка.УникальныйИдентификатор());
						
					КонецЕсли;
					
				КонецЦикла;
				
				СтрокаФрагмента = ФрагментСловаряЗамен.Добавить();
				СтрокаФрагмента.Тип = ТипЗнч(НеразделенныеПредопределенныеТипа.Получить(0).Ссылка);
