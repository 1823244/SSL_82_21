...
Процедура ЗаблокироватьТекущуюОбластьДанных(Знач ПроверитьОтсутствиеДругихСеансов = Ложь, Знач РазделяемаяБлокировка = Ложь) Экспорт
	...
	Если СтандартныеПодсистемыПовтИсп.ЭтоПлатформа83БезРежимаСовместимости() И НЕ РазделяемаяБлокировка Тогда
		...
		АдминистрированиеОбласти = ПравоДоступа("АдминистрированиеДанных", Метаданные);
		
		Если АдминистрированиеОбласти Тогда
	...
	ИсключенияБлокировки = Новый Массив();
	Если Не РазделяемаяБлокировка Тогда
		ИсключенияБлокировки.Добавить(Метаданные.РегистрыСведений.БлокировкиСеансовОбластейДанных.ПолноеИмя());
	КонецЕсли;
	...
	Для Каждого ЭлементМодели Из МодельДанных Цикл
		...
		Если ИсключенияБлокировки.Найти(ПолноеИмяОбъектаМетаданных) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
// Обертка над функцией МонопольныйРежим() глобального контекста. При вызове пользователем с правом
//  на конфигурацию АдминистрированиеДанных, но без права МонопольныйРежим() возвращает результат
//  функции МонопольныйРежим(), полученный в привилегированном режиме.
//
