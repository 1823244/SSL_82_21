...
Процедура СжатьФайл(Знач ПутьКDBFФайлам, ИмяФайла, КаталогВременныхФайлов) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент() Тогда
		// Обход возможной зависимости от регистра символов
		Маска = "";
		Для Позиция = 1 По СтрДлина(Файл.Имя) Цикл
			Символ = Сред(Файл.Имя, Позиция, 1);
			ВСимвол = ВРег(Символ);
			НСимвол = НРег(Символ);
			Если ВСимвол = НСимвол Тогда
				Маска = Маска + Символ;
			Иначе
				Маска = Маска + "[" + ВСимвол + НСимвол + "]";
			КонецЕсли;
		КонецЦикла;
		Кандидаты = НайтиФайлы(ПутьКDBFФайлам, Маска);
		Файл = Кандидаты[0]
	КонецЕсли;
	...
	Если Файл.Существует() Тогда
		...
		ZIPФайл.Добавить(Файл.ПолноеИмя, РежимСохраненияПутейZIP.НеСохранятьПути, РежимОбработкиПодкаталоговZIP.НеОбрабатывать);
		ZIPФайл.Записать();
	КонецЕсли;
