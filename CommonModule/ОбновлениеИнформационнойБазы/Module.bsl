...
Функция ИнформационнаяБазаЗаблокированаДляОбновления(УчитыватьПривилегированныйРежим = Истина, Знач ПриЗапускеКлиентскогоПриложения = Ложь) Экспорт
	...
	Если Не НеобходимоОбновлениеИнформационнойБазы()
...
Функция ВыполнитьОбновлениеИнформационнойБазы(ИсключениеПриНевозможностиБлокировкиИБ = Истина,
                                              Знач ПриЗапускеКлиентскогоПриложения = Ложь,
			...
			Иначе
				...
				Если БазаФайловая
					...
					Если Найти(ПараметрЗапускаКлиента, "РегламентныеЗаданияОтключены") = 0 Тогда
						Возврат "ПрекратитьРаботуСистемы";
					КонецЕсли;
	...
	ВыводитьОписаниеОбновлений =  Не НачальноеЗаполнениеДанных;
...
Функция ИтерацииОбновления() Экспорт
	...
	Для каждого ИмяПодсистемы Из ОписанияПодсистем.Порядок Цикл
		...
		ИтерацияОбновления.Вставить("ИмяОсновногоСерверногоМодуля", ОписаниеПодсистемы.ОсновнойСерверныйМодуль);
	...
	Если ИтерацияОбновлениеОсновнойПодсистемы = Неопределено Тогда
		...
		ИтерацияОбновления.Вставить("ВыполненныеОбработчики", Неопределено);
		ИтерацияОбновления.Вставить("ИмяОсновногоСерверногоМодуля", "ОбновлениеИнформационнойБазыПереопределяемый");
...
Процедура ОпределитьВыводОписанияОбновлений(ВыводитьОписаниеОбновлений)
	...
	Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		МенеджерКонстанты = Константы.СведенияОбОбновленииИБ.СоздатьМенеджерЗначения();
		МенеджерКонстанты.Значение = Новый ХранилищеЗначения(СведенияОбОбновленииИБ);
...
Функция ПоказатьОписаниеИзмененийСистемы()
	...
	СведенияОбОбновлении = СведенияОбОбновленииИнформационнойБазы();
	Если СведенияОбОбновлении <> Неопределено И СведенияОбОбновлении.Свойство("ВыводитьОписаниеОбновлений")
		И СведенияОбОбновлении.ВыводитьОписаниеОбновлений = Ложь Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ВыполненныеОбработчикиПрошлыхВерсий(НачальноеЗаполнениеДанных)
	...
	Если СведенияОбОбновлении <> Неопределено
		И Не СведенияОбОбновлении.ОтложенноеОбновлениеЗавершеноУспешно = Истина
...
Функция НовыеСведенияОбОбновлении(НачальноеЗаполнениеДанных)
	...
	СведенияОбОбновлении.Вставить("ВыводитьОписаниеОбновлений");
